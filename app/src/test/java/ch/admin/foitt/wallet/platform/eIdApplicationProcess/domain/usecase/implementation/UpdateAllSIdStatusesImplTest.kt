...
class UpdateAllSIdStatusesImplTest {
    ...
    @Test
    fun `If getAllCaseIds returns an error verify that update and fetchIdStatus are never called`() = runTest {
        val exception = IllegalStateException("error in db")
        coEvery { mockEIdRequestStateRepository.getAllCaseIds() } returns Err(EIdRequestError.Unexpected(exception))
